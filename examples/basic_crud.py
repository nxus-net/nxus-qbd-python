"""Basic CRUD lifecycle for vendors using the Nxus QuickBooks Desktop SDK.

Demonstrates:
  - Creating a vendor with sample data
...
def main() -> None:
    ...
    # The context manager automatically closes the underlying HTTP connection
    # pool when the block exits — even if an exception is raised.
    with NxusClient(**client_options()) as client:
        try:
            ...
            # You can pass a typed request model...
            ...
            # Responses are now typed Pydantic models — use snake_case attrs.
            vendor_id = vendor.id
            ...
            # ---------------------------------------------------------------
            print("\nRetrieving vendor...")
            fetched = client.vendors.retrieve(vendor_id, connection_id=connection_id)
            ...
            # ---------------------------------------------------------------
            print("\nDeleting vendor...")
            client.vendors.delete(vendor_id, connection_id=connection_id)
            print(f"  Deleted vendor {vendor_id}")

            print("\nCRUD lifecycle complete.")

        except NxusApiError as exc:
            # NxusApiError gives structured access to error details.
